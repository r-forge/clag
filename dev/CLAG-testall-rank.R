...
RES <- CLAG.clust(DATA2, analysisType=2, normalization="rank-column")
PCA <- prcomp(DATA2)
clusterColors <- c("black", rainbow(RES$ncluster))
plot(PCA$x[,1], PCA$x[,2], col=clusterColors[RES$cluster+1], main=paste(RES$nclusters, "clusters"))
...
M <- GLOBINE$M
RES <- CLAG.clust(M, delta=0.2, threshold=0.5, analysisType=3, normalization="rank-column")
o <- order(RES$cluster)
M2 <- RES$A[o,o]
...
M <- GLOBINE$M
M <- M[1:30,]
RES <- CLAG.clust(M, delta=0.2, threshold=0.5, analysisType=3, rowIds=1:30, normalization="rank-column")
o <- order(RES$cluster)
M2 <- RES$A[o,o]
clusterColors <- c("black", rainbow(RES$nclusters))[RES$cluster[o]+1]
colorScale <- colorRampPalette(c("blue", "green","yellow","red","darkred"))(1000)
