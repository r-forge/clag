...
open (FILEOUT_COMMAND, ">".$fileCommand)|| die "ERROR: CEBA couldn't write into the file $fileCommand";
	...
	while ($line =<FILE>)
	{
		...
		if ($line =~ /^ECART/ ){}
			...
			if($pairDataClusterValue[3] eq 1){
			$hashCluster{$indexCluster}=$pairDataClusterValue[2];
			$indexCluster++;
		}	
		}
	}
	...
	@keysA =();
		...
		$file="$folder/PR-SCORING-output-Matrix-Clusterized-Scores1.txt";
